...
bool Map::load(const std::string& path)
{
    ...
    //find the renderable_map node and bail if it doesn't exist
    auto mapNode = doc.child("map");
    if (!mapNode)
    {
        Logger::log("Failed opening renderable_map: " + path + ", no renderable_map node found", Logger::Type::Error);
        return reset();
    }
...
bool Map::loadFromString(const std::string& data, const std::string& workingDir)
{
    ...
    if (!result)
    {
        Logger::log("Failed opening renderable_map", Logger::Type::Error);
        Logger::log("Reason: " + std::string(result.description()), Logger::Type::Error);
        return false;
    }
    ...
    //find the renderable_map node and bail if it doesn't exist
    auto mapNode = doc.child("renderable_map");
    if (!mapNode)
    {
        Logger::log("Failed opening renderable_map: no renderable_map node found", Logger::Type::Error);
        return reset();
    }
...
{
    //parse renderable_map attributes
    ...
    {
        Logger::log("Invalid renderable_map version value, renderable_map not loaded.", Logger::Type::Error);
        return reset();
    }
    ...
    {
        Logger::log("Missing renderable_map orientation attribute, renderable_map not loaded.", Logger::Type::Error);
        return reset();
    }
    ...
    //this property is optional for older version of renderable_map files
    if (!attribString.empty())
    {
    ...
    {
        Logger::log("Invalid renderable_map tile count, renderable_map not loaded", Logger::Type::Error);
        return reset();
    }
    ...
    {
        Logger::log("Invalid tile size, renderable_map not loaded", Logger::Type::Error);
        return reset();
    }
    ...
    {
        Logger::log("Invalid he side length found, renderable_map not loaded", Logger::Type::Error);
        return reset();
    }
    ...
    // fill animated tiles for easier lookup into renderable_map
